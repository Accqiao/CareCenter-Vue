        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          ...
          <el-form-item label="照片" prop="staffImg">
            <el-upload
              ...
              <img v-if="ruleForm.staffImg" :src="`http://localhost:8080/${ruleForm.staffImg}`" class="avatar">
...
export default {
  ...
  created(){
    ...
    console.log(this.$route.query.staffId)
    console.log(this.ruleForm.staffId)
    this.getData();
  },
  methods:{
    getData(){
      let path = `http://localhost:8080/staff/findById/${this.ruleForm.staffId}`
    ...
    submitForm(formName){
      this.$refs[formName].validate((valid) => {
        if(valid){
          let path = 'http://localhost:8080/staff/updateByCondition';
