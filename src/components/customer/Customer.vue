    ...
    <el-card class="box-card">
      ...
      <div>
        <el-table
            ...
            <template slot-scope="scope">
              <div class="wrap">
                ...
                <div v-if="scope.row.photo">
                  <img :src="`http://localhost:8080/${scope.row.photo}`" class="image"/>
                </div>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="illness" label="健康状况" width="120"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit(id) {
      this.$router.push({path: "/CustomerUpdate", query: {cid: id}})
    },
    handleDelete(id) {
      let path = `http://localhost:8080/customer/delete/${id}`
      console.log(path)
